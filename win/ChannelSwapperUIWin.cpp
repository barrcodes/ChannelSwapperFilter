...
//
//-------------------------------------------------------------------------------
DLLExport BOOL WINAPI ChannelSwapperProc(HWND hDlg, 
								   UINT wMsg, 
								   WPARAM wParam, 
								   LPARAM lParam)
{
		...
		case WM_COMMAND:
			...
			switch (item)
			{
				...
				case kDAlpha:
					if (cmd == BN_CLICKED)
					{
						bool selected = FlipMaskBit(gParams->channelMask, item - kDRed);
						CheckDlgButton(
							hDlg,
							item,
							selected);
...
//-------------------------------------------------------------------------------
//
// Initialize the checboxes to be active if the mask bit is active for that item
// 
//-------------------------------------------------------------------------------
void InitColorCheckboxes(HWND hDlg)
{
	for (uint16 item = kDRed; item <= kDAlpha; item++)
	{
		bool selected = TestMaskBit(gParams->channelMask, item - kDRed);
		CheckDlgButton(
			hDlg,
			item,
			selected);
...
Boolean DoUI(void)
{
	...
	INT_PTR result = DialogBoxParam(GetDLLInstance(),
							(LPSTR)"CHANNELSWAPPERDIALOG",
							(HWND)platform->hwnd,
							(DLGPROC)ChannelSwapperProc,
							NULL);
	//used by the ChannelSwapperProc routine, don't let the error go further
	*gResult = noErr;
	return (result == kDOK);
}
...
// UpdateProxyItem
//
// Force the WM_PAINT message to the ChannelSwapperProc routine.
//
//-------------------------------------------------------------------------------
void UpdateProxyItem(HWND hDlg)
{
