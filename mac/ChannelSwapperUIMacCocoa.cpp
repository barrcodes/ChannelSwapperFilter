...
#include "ChannelSwapperController.h"
...
Boolean DoUI (void) 
{
    ...
    OSStatus err = initializeCocoaChannelSwapper();

	// call function to show window
	// it's not an error 1 = ok, 0 = cancel
    if (!err)
		err = orderWindowFrontChannelSwapper();
	else
...
void DoAbout(void)
{
	...
	[alert setMessageText:@"About ChannelSwapper"];
...
// end ChannelSwapperUIMacCocoa.cpp 
